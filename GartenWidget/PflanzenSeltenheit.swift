...
public enum PflanzenSeltenheit: String, Codable, CaseIterable {
    case bronze  = "bronze"
    case silber  = "silber"
    case gold    = "gold"
    case diamant = "diamant"

    public var lokalisiertTitel: String {
        return NSLocalizedString("rarity.\(rawValue)", comment: "")
    }

    public var farbe: Color {
        switch self {
    ...
    /// Nächste Seltenheit
    public var naechste: PflanzenSeltenheit? {
        switch self {
    ...
    public func xpBisNaechste(aktuelleXP: Int) -> Int? {
    ...
    public func fortschritt(aktuelleXP: Int) -> Double {
        let von = xpSchwelle
