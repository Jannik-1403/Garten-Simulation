...
- **Automatischer Fokus-Timer-Pause**: Wenn die App in den Hintergrund geht (oder gesperrt wird), pausiert der Fokus-Timer automatisch (`isTimerRunning = false`). Die Live Activity auf dem Sperrbildschirm wechselt in einen statischen Pause-Modus und friert bei der verbleibenden Restzeit (z. B. "20:02") ein, statt weiterzulaufen. Kehrt die App in den Vordergrund zurück, läuft der Timer nahtlos weiter.
- **Fokus-Sitzung Wiederherstellung bei App-Neustart**: Die komplette Fokus-Sitzung (inklusive Restzeit, ausgewählten Zielen/Unterzielen und Status) wird in den `UserDefaults` persistent zwischengespeichert. Wenn die App vom System oder Benutzer komplett geschlossen wurde, erkennt die App dies beim nächsten Start, öffnet automatisch die Detailansicht der betroffenen Pflanze, stellt das Fokus-Fenster wieder her und lässt den Timer genau dort fortsetzen, wo er unterbrochen wurde.
