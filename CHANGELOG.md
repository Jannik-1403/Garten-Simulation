...
- **Feedback-Update:** Das uneingeschränkte "Top"-Lob erscheint jetzt konsequent nur noch bei perfekten 100%. Ab 99% abwärts bekommt man wieder konstruktive Tipps angezeigt. Außerdem steht nun auch bei 100% die Info, wann man im Schnitt ins Bett geht und wann man aufstehen sollte.
...
## 2026-08-30 14:42:20 - Brustumfang & Info-Texte
- Neuer Körperumfang 'Brustumfang' hinzugefügt.
- Info-Texte (i-Icon) mit genauen Messanleitungen für alle Körperumfänge eingebaut.
- Alle neuen Strings zu 100% lokalisiert (inklusive Fallback).
...
- **Routinen (Custom To-Dos)**: Das Beschreibungsfeld bei der Erstellung von To-Dos in Routinen wurde wieder hinzugefügt. Die Beschreibung kann nun auch im Nachhinein über den "Bearbeiten"-Dialog angepasst werden. Die Löschen-Aktion (Swipe) ist nun in einem angenehmeren Rot-Ton gehalten und schließt den Bearbeitungsmodus nicht ungewollt ab.
- **Routine UI Optimierungen**: In der Bearbeitungsansicht einer Routine wurde der störende Text 'Reihenfolge anpassen' und der 'Sortieren'-Button komplett entfernt. Um in den Bearbeitungs-Modus zu gelangen (und Elemente links über das Minus zu löschen oder zu sortieren), reicht es nun aus, einfach lange auf ein Element zu drücken. Die vorherige Wischgeste zum Löschen wurde komplett entfernt. Zum Anpassen der To-Dos (inkl. Beschreibung) wischt man weiterhin bei diesen Elementen, bei Gewohnheiten gibt es keine Wischgesten mehr. Das fälschlicherweise hinzugefügte Beschreibungsfeld bei der Erstellung von eigenen Gewohnheiten wurde wieder restlos entfernt.
- **Routine UI Optimierungen**: In der Bearbeitungsansicht einer Routine wurde die Swipe-Geste komplett entfernt. Stattdessen kann nun durch einfaches Antippen der Name aller Elemente (sowohl To-Dos als auch Gewohnheiten) spezifisch für diese Routine angepasst werden. Die Beschreibung lässt sich dabei nur bei To-Dos ändern. Zudem wurde der Bearbeitungsmodus zum Sortieren und Löschen auf ein langes Gedrückthalten gelegt, damit die störenden 'Sortieren'-Buttons und SwipeActions entfallen.
- **Pflanzendetails**: Das Eingabefeld für eine eigene Beschreibung in der Detailansicht von Gewohnheiten/Pflanzen wurde vollständig entfernt.
- **Routine UI Optimierungen**: In der Bearbeitungsansicht einer Routine wurde die Swipe-Geste nun final komplett entfernt. Stattdessen kann durch einfaches Antippen der Name aller Elemente (sowohl To-Dos als auch Gewohnheiten) spezifisch für diese Routine angepasst werden. Die Beschreibung lässt sich dabei nur bei To-Dos ändern. Der Text in diesem Dialog wurde stark vereinfacht, um klar zu differenzieren, ob man eine Gewohnheit oder Aufgabe bearbeitet. Zudem wurde der Bearbeitungsmodus zum Sortieren und Löschen auf ein langes Gedrückthalten gelegt.
...
- **Tages-Score Berechnung:** Der Tages-Score springt morgens nicht mehr direkt auf 50%. Anstatt Aufgaben wie Ernährung oder Trinken pauschal mit 50% zu bewerten (nur weil sie auf 'Warnung' stehen), wird ab sofort der **exakte prozentuale Fortschritt** (z.B. getrunkene ml im Verhältnis zum Tagesziel) verwendet. Dadurch baut sich der Score jetzt realistisch und stetig über den Tag auf.
