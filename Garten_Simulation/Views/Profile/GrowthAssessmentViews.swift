...
struct GrowthAssessmentQuizView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if showResult {
                GrowthResultView(
                    result: assessmentStore.growthResult!,
                    onRetake: {
                        assessmentStore.resetGrowthResult()
                        resetQuiz()
                    }
                )
    ...
    private var growthQuizBody: some View {
        VStack(spacing: 0) {
...
// MARK: - Growth Result Screen

struct GrowthResultView: View {
    let result: GrowthAssessmentResult
    let onRetake: () -> Void

    @EnvironmentObject var settings: SettingsStore
    @Environment(\.dismiss) private var dismiss

    private var profile: GrowthProfile { result.profile }
                ...
                VStack(spacing: 28) {
                    ...
                    VStack(spacing: 16) {
                        Item3DButton(
                            ...
                            Image(HabitCategory.growth.assetName)
                                .resizable()
                                .scaledToFit()
                    ...
                    // Score Bars
                    GrowthScoreBreakdownCard(result: result)
                        .padding(.horizontal, 20)
...
// MARK: - Growth Score Breakdown Card

struct GrowthScoreBreakdownCard: View {
    let result: GrowthAssessmentResult
