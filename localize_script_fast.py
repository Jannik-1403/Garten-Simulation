...
def translate_single(key, de_text, xc_lang, google_lang):
    try:
        ...
        return (key, xc_lang, translated)
    except Exception as e:
        return (key, xc_lang, None)
...
for xc_lang, google_lang in lang_mapping.items():
    for key, de_text in translations_to_add.items():
        ...
        if xc_lang not in locs or locs[xc_lang]["stringUnit"]["state"] != "translated":
            tasks.append((key, de_text, xc_lang, google_lang))

print(f"Total translations needed: {len(tasks)}")

total_done = 0
failed = 0

with ThreadPoolExecutor(max_workers=50) as executor:
    futures = [executor.submit(translate_single, t[0], t[1], t[2], t[3]) for t in tasks]
    for future in as_completed(futures):
        key, xc_lang, res = future.result()
        if res is not None:
        ...
        if (total_done + failed) % 100 == 0:
            print(f"Progress: {total_done + failed}/{len(tasks)} (Failed: {failed})")

with open(xcstrings_path, "w", encoding="utf-8") as f:
    json.dump(xc_data, f, indent=2, ensure_ascii=False)

print(f"Done! Added {total_done} translations. Failed: {failed}")
