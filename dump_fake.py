...
for key, value in data["strings"].items():
    ...
    if zh == source_text and any(c.isalpha() for c in source_text):
        if source_text not in ["XP", "x%@", "h", "min"]:
            fake_dict[key] = source_text

with open("fake_strings.json", "w") as f:
    json.dump(fake_dict, f, ensure_ascii=False, indent=2)

print(f"Dumped {len(fake_dict)} fake strings.")
